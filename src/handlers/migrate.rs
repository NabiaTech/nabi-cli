...
pub fn migrate_directory(
    ...
    validate_dir_name(&config.state_dir).context("Invalid source directory name")?;
    ...
    // Resolve XDG paths
    let state_base = NabiPaths::state_dir().context("Failed to resolve XDG_STATE_HOME")?;
    let data_base = NabiPaths::data_dir().context("Failed to resolve XDG_DATA_HOME")?;
    ...
    if entries.is_empty() {
        println!(
            "  {} Source directory is empty (nothing to migrate)",
            "ℹ".blue()
        );
        return Ok(MigrationStats::default());
    }
    ...
    let total_size =
        calculate_size(&source_dir).context("Failed to calculate source directory size")?;

    println!(
        "  Found {} files to migrate (Total size: {})",
    ...
    // Detect conflicts
    let conflicts =
        detect_conflicts(&source_dir, &dest_dir).context("Failed to detect conflicts")?;
    ...
    if !conflicts.is_empty() {
        ...
        if !options.force {
            ...
            println!(
                "  {} --force enabled, will overwrite conflicts",
                "⚠".yellow()
            );
        }
    }
    ...
    for entry in WalkDir::new(&source_dir).min_depth(1) {
        ...
        if entry.file_type().is_file() {
            ...
            if options.force && dest_file.exists() {
                fs::remove_file(&dest_file).context(format!(
    ...
    if stats.files_failed > 0 {
        eprintln!(
            "  {} Migration incomplete: {} files failed to move",
            "✗".red(),
            stats.files_failed
        );
    ...
    if remaining.is_empty() {
        fs::remove_dir(&source_dir).context(format!(
        ...
        println!(
            "  {} Source directory not empty, leaving in place",
            "⚠".yellow()
        );
        if options.verbose {
...
pub fn migrate_directories(
    ...
    println!(
        "Migrating {} directory(ies) from XDG_STATE_HOME to XDG_DATA_HOME\n",
    ...
    println!(
        "  Total files moved: {}",
        total_stats.files_moved.to_string().bright_white()
    );
    println!(
        "  Total size: {}",
        format_size(total_stats.total_size_bytes).bright_white()
    );
...
pub fn handle_migrate_verify(dirs: &[String], _verbose: bool) -> Result<()> {
    ...
    for state_dir in dirs_to_check {
        if let Some(data_dir) = get_dest_dir(&state_dir) {
            ...
            if source_exists {
                println!(
                    "  {} Migration may be incomplete - source still exists",
                    "⚠".yellow()
                );
            }
